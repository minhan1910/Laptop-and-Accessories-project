                        ...
                        <div class="x_panel">
                            ...
                            <div class="x_content">
                                ...
                                <form class="form-horizontal form-label-left"
                                    action="{{ route('admin.products.update', ['id' => $productInfo->id]) }}" method="POST"
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9 col-sm-9 ">
                                            <textarea class="form-control" rows="3" placeholder="Product Descripion..." id="content" name="content">
                                                {{ old('content') ?? $productInfo->content }}</textarea>
                                            @error('content')
                                                <span style="color:red">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="form-group row align-items-center">
                                        ...
                                        <img src="{{ $productInfo->feature_image_path }}" width="200px" height="200px"
                                            alt="">
                                                ...
                                                @if (!empty('categoryList'))
                                                    @foreach ($categoryList as $category)
                                                        <option value="{{ $category->id }}"
                                                            {{ old('category_id') ?? $productInfo->category_id == $category->id ? 'selected' : '' }}>
                                                            {{ $category->name }}
                                                        </option>
                                                    @endforeach
                                                @endif
